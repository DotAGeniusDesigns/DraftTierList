...
const Navbar = ({ darkMode, onToggleDarkMode }) => {
    ...
    const themeButtonClass = `flex h-9 w-9 shrink-0 items-center justify-center rounded-xl transition sm:h-10 sm:w-10 ${
        darkMode
    ...
    const renderNavLink = (item, compact = false) => (
        <NavLink
            ...
            to={item.path}
            className={({ isActive }) =>
                `${ui.navPill(darkMode, isActive)} ${compact ? 'w-full flex-col gap-1 py-2.5' : ''}`
            }
            title={item.label}
        >
            ...
            <div className="container mx-auto max-w-7xl px-3 sm:px-4">
                <div className="flex items-center justify-between gap-3 py-3 sm:py-3.5">
                    ...
                    <button
                        type="button"
                        onClick={onToggleDarkMode}
                        className={`md:hidden ${themeButtonClass}`}
                        title={darkMode ? 'Switch to light mode' : 'Switch to dark mode'}
                    >
                        {darkMode ? (
                            <svg viewBox="0 0 20 20" fill="currentColor" className="h-5 w-5">
                ...
                <div className={`md:hidden border-t pb-3 pt-2.5 ${darkMode ? 'border-white/5' : 'border-slate-200/70'}`}>
